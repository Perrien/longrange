...
describe('cut layers (a real hole in the face)', () => {
  /** A windowed plate: white face, one isHole zone, cut last. */
  ...
  it('emits the cut zone geometry on BOTH faces, flagged cut', () => {
    // A hole through steel is a hole from either side, which is the same
    // both-faces rule every other layer follows.
    const cuts = only(ops(WINDOWED), 'ellipse');
    expect(cuts).toHaveLength(2);
    expect(cuts.every((o) => o.cut === true)).toBe(true);
    expect(cuts.map((o) => o.side).sort()).toEqual(['downrange', 'shooter']);
  });

  it('gives a cut NO fill and NO stroke — it removes, it does not draw', () => {
    for (const op of only(ops(WINDOWED), 'ellipse')) {
      expect(op.fill).toBeUndefined();
      expect(op.stroke).toBeUndefined();
      expect(op.strokeWidthPx).toBeUndefined();
    }
  });

  it('reuses the ZONE shape, so the drawn hole cannot drift from the scored one', () => {
    // The whole reason `cut` names zones instead of carrying its own geometry:
    // `isHole` (what the hit test misses through) and the visible opening are
    // one shape by construction.
    const zone = WINDOWED.zones[0].shape as { cx: number; cy: number; r: number };
    const expected = toPx({ x: zone.cx, y: zone.cy }, WINDOWED.aspect, 'downrange');
    const op = only(ops(WINDOWED), 'ellipse').find((o) => o.side === 'downrange')!;
    expect(op.cx).toBeCloseTo(expected.x, 9);
    expect(op.cy).toBeCloseTo(expected.y, 9);
    expect(op.rx).toBeCloseTo(zone.r * PX_PER_LOCAL_X, 9);
  });

  it('corrects a cut for the tile anisotropy, same as any other circle', () => {
    // Trap 1: a local circle is a texel ELLIPSE. A cut that skipped the
    // correction would punch an egg-shaped window on a tall target.
    const t: TargetType = {
      ...TALL,
      zones: [
        { id: 'hole', label: 'Hole', isHole: true, shape: { kind: 'circle', cx: 0, cy: 0, r: 0.2 } },
        ...TALL.zones,
      ],
      paint: { palette: { face: 0xffffff }, layers: [{ kind: 'fill', color: '$face' }, { kind: 'cut', zoneIds: ['hole'] }] },
    };
    const op = only(ops(t), 'ellipse')[0];
    expect(op.rx).toBeCloseTo(0.2 * PX_PER_LOCAL_X, 9);
    expect(op.ry).toBeCloseTo((0.2 / TALL.aspect) * PLATE_TILE_HEIGHT, 9);
    expect(op.ry).toBeLessThan(op.rx);
  });

  it('marks nothing as cut on a target with no cut layer', () => {
    for (const op of ops(TALL)) {
      if (op.kind === 'ellipse' || op.kind === 'polygon') expect(op.cut).toBeUndefined();
    }
  });

  it('throws on a cut naming a zone the type does not have', () => {
    const t: TargetType = {
      ...WINDOWED,
      paint: { palette: { face: 0xffffff }, layers: [{ kind: 'cut', zoneIds: ['nope'] }] },
    };
    expect(() => planFace(t)).toThrow(/cuts unknown zone 'nope'/);
  });
});
