...
> ### Decision — ONE terrain, TWO firing points (owner, 2026-07-28)
>
> The rimfire ladder shares the ELR range's hillside, forest and clearance solver, but
> gets **its own firing point at ground level** alongside the elevated ELR position.
>
> The conflict this resolves is real and would otherwise be invisible until it was
> built: from the elevated ELR eye, a 50 m target sits about **12° below** you. That is
> a strange rimfire stance, and it drags a genuine cosine error onto exactly the
> stations meant to teach a clean drop. Two firing points cost one extra spawn position
> and no extra terrain.
>
> What it keeps: **250 m is a shared station** — the far end of the rimfire ladder and
> the near end of the centerfire one. Shoot it with both rifles and the 4:1 scaling
> below stops being a table and becomes something you can see on one plate.
>
> Ladder: **50 m steps to 500** from the low line (10 stations), **250 m steps to 2000**
> from the high line (8 stations). The clearance solver runs per firing point, since a
> sight line is defined by the eye it starts from.

> ### Addition — a .22 LR near ladder, and why it is not a beginner mode (2026-07-28)
>
> Owner asked about a close strip for rimfire, 50 m steps out to 250. **Yes — and it
> earns its place for a better reason than "somewhere to start."**
>
> **The .22 LR is a 4:1 scale model of the centerfire problem.** Computed from the engine
> (ISA, 10 mph full-value crosswind, .22 zeroed 50 m / 6.5 CM zeroed 100 m):
>
> | | come-up | wind hold |
> |---|---|---|
...
>
> The firing solution the player has to build is *the same angular problem* at a quarter
> of the distance. This is exactly why rimfire trainers exist in reality, and it is now
> quantified from our own engine rather than asserted. It means the near strip teaches
> the ELR lesson in 250 m of ground, with no barrel life, no recoil, and — once a money
> or effort model exists — no cost. **Put the .22 ladder in the teaching sequence
> BEFORE the ELR ladder, not beside it as an easy option.**
>
> **Upper limit: 500 yd / ~450 m, not 300.** ⚠️ *An earlier draft of this section said
> ~300 m. That assumed DIAL-ONLY, which is the same mistake the main plan already
> corrects for the ELR ladder — holdover is a technique, not a gear gate. Owner flagged
> it, and real-world reports of .22 at 200–400 yd "with a LOT of holdover" are exactly
> right.* Full table, 50 m zero, 10 mph full-value crosswind:
>
...
>
> 34.56 MIL at 500 m against a 20 MIL base means ~15 MIL of holdover — available at 10×
> on an FFP reticle (~21 MIL below centre), so 500 m is reachable and is *supposed* to
> feel like work. **500 m is the right cap for a rimfire ladder**: past it the come-up
> outruns even holdover, and 5+ MIL of wind for a gentle breeze makes the wind call
> dominate everything else.
>
> **Supersonic rimfire exists, and it is a better teaching mechanic than the flat
> numbers suggest.** ⚠️ *The earlier claim that "the .22 has no transonic wall" is true
> only of the catalog's standard-velocity load.* High-velocity .22 LR leaves at **Mach
> 1.12** and is back through Mach 1 by **50 m** — it crosses the transonic band right in
> front of the shooter. It shoots visibly flatter (29.39 vs 34.56 MIL at 500 m) and in
> reality it groups *worse* for exactly that reason, which is why serious rimfire
> competitors buy subsonic match ammo. **The engine will not reproduce that today**: the
> transonic Cd rise is modelled (it shapes the trajectory), but dispersion comes only
> from MV SD, BC SD and rifle precision, so nothing makes an HV round scatter more
> through the transition. Logged in `Wiki/_gaps.md` — without it, "buy the faster ammo"
> is strictly correct in-game and strictly wrong in life.
>
> **.22 WMR is a genuinely different rifle**, not a faster .22 LR: supersonic to ~200 m,
> and 17.04 MIL at 500 m against the LR's 34.56. Worth a catalog slot.
>
> **Oracle coverage gap:** `validation/loads.json` samples the .22 only to **300 m**. A
> 500 m ladder puts the last four stations outside validated coverage. Extending is the
> same cheap data edit as the 2000 m extension (TOF at 500 m is 2.14 s, nowhere near the
> harness's 15 s wall) and should be done before the rimfire ladder ships.
>
> **No cut strip needed.** Running the clearance solver over 25/50/…/250 m against the
> same 4000-tree field returns **zero occluders at every station** — the near sight cone
> is short and narrow, so few trees fall inside it. The strip can be woods like the rest
> of the range.
>
> **One thing to fix before building it:** `DEFAULT_MARGIN_M = 2.0` in
> `sight-clearance.ts` is a long-range number. At 50 m a 1 MIL gong is **5 cm** and a
> flat 2 m margin demands a corridor forty times the plate. It still solves, but the
> margin should scale with the plate (≈2× plate radius) rather than sit fixed, or the
> near ladder is being placed against a rule that means something different there.
