...
- (1.2) **Range A targets are MOA-sized (owner decision 2026-07-14): 2 / 1.5 / 1
  MOA on every rack**, so each rack is equally hard to hit and challenge scales
  with range via wind/drop, not target size (`PLATE_MOA` in range-a-config.ts).
  Physical `diameterM = moaToRad(moa) × distanceM` (box-true, no hidden values).
  Berm/rack dims scale from the plates; low-wide berm proportions ported from
  steel-sim `Berm.js`. Easy to retune the MOA set once the scope pipeline (1.3)
  and shot loop (1.4/1.6) show what plays well. Round plates only (no ovals).
